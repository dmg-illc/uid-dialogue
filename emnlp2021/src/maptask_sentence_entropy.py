...
def compute_entropy(args, dataframe):
    ...
    if args.skip_speaker_ids:
        speaker_ids = tokenizer.convert_tokens_to_ids(['g', 'f'])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--max_seq_len", default=150, type=int,
        help="The maximum number of input tokens."
    )
    parser.add_argument(
        "--add_speaker_ids", action='store_true',
        help="Whether to prepend utterances with speaker identifiers (e.g. 'g: yeah')"
    )
    parser.add_argument(
    ...
    out_file_name = compute_entropy(args, dataframe[:5])
